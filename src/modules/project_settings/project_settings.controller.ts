...
export class ProjectSettingController {
  ...
  toggleProjectSettings = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { statusCode = null, ...others } = await this.projectSettingService.toggleProjectSettings(company_id, payload);
  ...
  getProjectSettingDetails = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    const { statusCode = null, ...others } = await this.projectSettingService.getProjectSettingDetails(company_id);

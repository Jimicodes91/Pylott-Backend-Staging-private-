...
export const signIn = async (data: loginData) => {
	...
	try {
		...
		let token = jwt.sign({ email: user.email, role: user.role, _id: user._id }, process.env.JWT_SECRET as string, { expiresIn: '1d' });
...
export const sendInvitation = async (adminId: string, email: string, role: UserRole) => {
	try {
		...
		const registrationLink = `${process.env.FRONTEND_URL}/register?token=${invitationToken}&email=${encodeURIComponent(email)}&role=${role}&company=${admin.company}`;
...
export const addClientService = async (name: string, email: string, companyId: string) => {
	try {
		const existingUser = await User.findOne({ email });
		if (existingUser) {
			throw new HttpError('Email is already registered', 400);
...
export const updatePasswordService = async (userId: string, currentPassword: string, newPassword: string) => {
	try {
